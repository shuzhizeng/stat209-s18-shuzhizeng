...
model_h <- lm_basic(self_reliance ~ 1, data = hungary)
...
**Answer**: The range of CI in Hungary is larger since the dataset becomes smaller. 
...
```{r}
model_gender <- lm_basic(self_reliance ~ 1+ gender, data = hungary)
...
```{r}
model_differ <- lm_basic(self_reliance ~ 1+ fourties + age , data = age)
reg_table(model_differ, level = 0.95)
```
