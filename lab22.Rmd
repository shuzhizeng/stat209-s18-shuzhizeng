...
model_normal <- lm_basic(perfectionism ~ 1 + speed_normal, data = pf)
reg_table(model_normal, level = 0.95)
...
**Answer**: Some people pratice a lot so they would do quicker while others may not familiar with the concepts so they do slower(More than 1 hour). 
...
```{r}
model_contrast <- lm_basic(sensitivity ~ 1 + emotional_stability+ reasoning, data = pf)
reg_table(model_contrast, level = 0.95)
```
...
```{r}
model_contrast <- lm_basic(sensitivity ~ 1 + emotional_stability+ reasoning+speed+gender+fct_lump(country, 5), data = pf)
reg_table(model_contrast, level = 0.95)
```
...
**Answer**: Reasoning has the greatest average score while sensitivity has the least score. 
