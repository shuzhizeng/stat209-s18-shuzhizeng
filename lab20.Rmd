...
**Answer**:
The intercept of the regression line is -2256 and the slope is 7756.
...
```{r}
modelci <- lm_basic(price ~ 1 + carat, data = diamonds)
reg_table(modelci, level=0.95)
```
...
**Answer**: The term "m" in the formula of regressio line y=mx+b.
...
```{r}
modeldep <- lm_basic(price ~ 1 + depth, data = diamonds)
reg_table(modeldep, level=0.95)
```

What does the model predict will be the price of a diamond with a depth of
60?

**Answer**: -29.65*60+5763.67
...
**Answer**: The diamond will be smaller. 
...
**Answer**: 4045.3+-102.2*50+7765.1*2. 

Are both of the slopes in the model statistically significant? What are
their signs?

**Answer**: Yes. One positive and one negative. 
...
model <- lm_basic(price ~ 1 + depth + carat, data = diamonds)
reg_table(model, level = 0.95)
pred <- add_prediction(diamonds, model)
```
...
**Answer**: "m" in y=mx+b (regression line).
...
**Answer**: based on the slope. Normally two times. 
