...
**Answer**: The dataset burrito is included in restaurants.

Now fit a linear regression that predicts the yelp score as a function of
whether the burrito includes chips.

```{r}
yelps<- lm_basic(yelp~ 1+ chips_included, data= burrito)
reg_table(yelps, level=0.95)
```

Describe what the slope coefficient in the previous model really means
in terms of the data:

**Answer**: The mean of yelp is 3.91 while the mean of yelp with chips is around 3.91 +0.06. There's no evidence that the mean of these two variables is different. 
...
**Answer**: Zero is included between -0.21-0.32. 
...
```{r}
syn<- lm_basic(yelp~1+ high_synergy, data= burrito)
reg_table(syn, level=0.95)
```
...
```{r}
overall<- lm_basic(overall~1+ high_synergy, data= burrito)
reg_table(overall, level=0.95)
```
...
```{r}
overall_syn<- lm_basic(overall~1+ synergy, data= burrito)
reg_table(overall_syn, level=0.95)
```
