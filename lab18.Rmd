...
**Answer**: No. Most fall in between 60000-70000. 
...
```{r}
model_m <- lm_basic(base~ 1, data=masters)
reg_table(model_m, level = 0.95)
```
...
```{r}
model_l <- lm_basic(age~ 1, data=london)

reg_table(model_l, level = 0.8)
```

Make sure you actually extract the answer here:

**Answer**: 33.86 with CI (33.07-34.65).

Describe in words how the proceeding confidence interval should be
interpreted.

**Answer**: The 80% chance that estimator of average age falls in the range of 33.07-34.65.
